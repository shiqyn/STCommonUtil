...
+ (void) addMJNotifierWithText : (NSString* ) text dismissAutomatically : (BOOL) shouldDismiss {
    ...
    if(notifierView) {
        ...
        [self updateNotifierWithAnimation:notifierView withText:text completion:^(BOOL finished) {
            ...
            [UIView animateWithDuration:0.4 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut|UIViewAnimationOptionAllowUserInteraction
                             animations:^{
        ...
        if(shouldDismiss) {
            [self performSelector:@selector(dismissMJNotifier)
                       withObject:nil
                       afterDelay:2.0
        ...
        if(shouldDismiss) {
            [self performSelector:@selector(dismissMJNotifier) withObject:nil afterDelay:2.0 inModes:[NSArray arrayWithObject:NSRunLoopCommonModes]];
        }
        else {
